...
function getFizzBuzz(num) {
  if (num / num === 1 && num % 3 !== 0 && num % 5 !== 0) return num;
...
function doRectanglesOverlap(rect1, rect2) {
  const topOverlap = Math.max(
    0,
    Math.min(rect1.top + rect1.width, rect2.top + rect2.width)
      - Math.max(rect1.top, rect2.top),
  );
  const leftOverlap = Math.max(
    0,
    Math.min(rect1.left + rect1.height, rect2.left + rect2.height)
      - Math.max(rect1.left, rect2.left),
  );

  return topOverlap * leftOverlap > 0;
}
...
function findFirstSingleChar(str) {
  return str
    .split('')
    .filter(
      (character, index, obj) => obj.indexOf(character) === obj.lastIndexOf(character),
    )
    .shift();
}
...
function evaluateTicTacToePosition(/* position */) {
  ...
  // if (/'x''x''x'|'x'...'x'...'x'|'x'....'x'....'x'|'x'..'x'..'x'/.test(value)) return 'X';
  // if (/000|0...0...0|0....0....0|0..0../.test(value)) return '0';
  // // if (/0/.test(value)) return -1;
  // return 0;
}
